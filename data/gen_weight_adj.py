...
def parse_args():
    ...
    parser.add_argument("weight_adj_path", type=str, help="Path to the weight_adj file.")
    return parser.parse_args()
args = parse_args()
fold_info_path = args.fold_info_path  # 从命令行传入的路径
weight_adj_path = args.weight_adj_path
...
for i in range(5):
    ...
    score = np.zeros_like(adj_np, dtype=np.float64)
    for j in range(5):
        print(f'j{j}')
